...
export default function AdminSettingsPage() {
  ...
  const settingsQuery = useQuery({
    ...
    queryFn: async () => {
      const res = await supabaseClient.from("app_settings").select("*");
      const result = res.data;
      if (!res.data) {
        return { notConfigured: true };
      }
      return { item: result[0] as AppSettings | null };
    },
  });
  ...
  const saveSettingsMutation = useMutation({
    mutationFn: async () => {
      ...
      const res = await supabaseClient.from("app_settings").upsert(settings);
      const result = res.data;
      return result[0] as AppSettings;
    },
  ...
  const addAdminMutation = useMutation({
    mutationFn: async () => {
      ...
      const res = await supabaseClient.from("admins").insert({ email });
      const result = res.data;
      return result[0] as { user_id: string; email: string };
    },
    onSuccess: () => {
      setNewAdminEmail("");
  ...
  const createUserMutation = useMutation({
    mutationFn: async () => {
      ...
      const res = await supabaseClient.auth.api.createUser({
        email,
        password,
        metadata: { username },
      });
  ...
  const removeAdminMutation = useMutation({
    mutationFn: async (userId: string) => {
      const res = await supabaseClient.from("admins").delete().eq("user_id", userId);
      const result = res.data;
      return true;
    },
    onSuccess: () => {
  ...
  const updateAdminMutation = useMutation({
    mutationFn: async ({ userId, email }: { userId: string; email: string }) => {
      const res = await supabaseClient.from("admins").update({ email }).eq("user_id", userId);
      const result = res.data;
              ...
              <div className="flex items-center gap-6">
                <label className="inline-flex items-center">
                  <input
                    ...
                    value="single"
                    className="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300"
                    checked={currentSettings.enable_multi_exam === false}
                    onChange={() => updateSetting("enable_multi_exam", false)}
                  />
                  <span className="ml-2 block text-sm text-gray-700">Single Exam Mode</span>
                </label>
                <label className="inline-flex items-center">
                  <input
                    type="radio"
                    ...
                    className="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300"
                    checked={currentSettings.enable_multi_exam !== false}
                    onChange={() => updateSetting("enable_multi_exam", true)}
                  />
                  <span className="ml-2 block text-sm text-gray-700">Multi-Exam Mode</span>
                </label>
              </div>
            </div>
            <p className="text-xs text-gray-500 mt-2">
              Single mode: Students directly access the single published exam. Multi mode: Students enter code to see all available exams.
            </p>
          </div>
          ...
          <div className="md:col-span-2">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
              <div>
                ...
                <input
                  ...
                  max="10"
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                  value={currentSettings.code_length ?? 4}
                  onChange={(e) => updateSetting("code_length", parseInt(e.target.value) || 4)}
                />
              </div>
              <div>
                <label className="block text-xs font-medium text-gray-600 mb-1">Format Type</label>
                <select
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                  value={currentSettings.code_format ?? "numeric"}
                  onChange={(e) => updateSetting("code_format", e.target.value)}
                >
                  <option value="numeric">Numbers Only (0-9)</option>
                  <option value="alphabetic">Letters Only (A-Z)</option>
              ...
              <div>
                <label className="block text-xs font-medium text-gray-600 mb-1">
                  Custom Pattern {currentSettings.code_format !== "custom" && "(Optional)"}
                </label>
                <input
                  type="text"
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                  value={currentSettings.code_pattern ?? ""}
                  onChange={(e) => updateSetting("code_pattern", e.target.value)}
                  placeholder="e.g., NN-AA-NN"
                  disabled={currentSettings.code_format !== "custom"}
                />
              </div>
            </div>
            <div className="bg-blue-50 border border-blue-200 rounded-lg p-3">
              ...
              <ul className="text-xs text-blue-700 space-y-1">
                <li>• <code>NNNN</code> = 4 numbers (1234)</li>
                <li>• <code>AAAA</code> = 4 letters (ABCD)</li>
                <li>• <code>NN-AA</code> = 2 numbers + dash + 2 letters (12-AB)</li>
                <li>• <code>AAA###</code> = 3 letters + 3 numbers (ABC123)</li>
              </ul>
              <p className="text-xs text-blue-600 mt-2">
                Use N for numbers, A for letters, # for any character, and other characters as literals.
              </p>
            </div>
          </div>
